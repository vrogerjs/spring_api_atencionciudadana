...
public interface AtencionRepository extends JpaRepository<Atencion, Long> {
    ...
    @Query("SELECT DISTINCT a.dependencia.name, COUNT(a.dependencia.id) FROM Atencion a WHERE a.activo=0 AND a.fecha BETWEEN :fechaIni AND :fechaFin GROUP BY a.dependencia.id")
    abstract List getCountByDependencia(LocalDate fechaIni, LocalDate fechaFin);

    @Query("SELECT a.dependencia.name, COUNT(a.dependencia.id), a.fecha FROM Atencion a WHERE a.activo=0 AND a.fecha BETWEEN :fechaIni AND :fechaFin AND a.dependencia.id=:IdDependencia GROUP BY a.fecha")
